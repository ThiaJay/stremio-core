    ...
    fn update(&mut self, msg: &Msg, ctx: &Ctx) -> Effects {
        match msg {
            ...
            Msg::Action(Action::Player(ActionPlayer::Ended)) if self.selected.is_some() => {
                ...
                // A genuine Ended event is stronger completion evidence than a seek.
                // If playback reaches Ended after seeking into the credits, establish watched
                // state before the existing unload logic clears the stale resume position.
