...
function createPlaybackHealthObservation(input) {
    ...
    return {
        ...
        profile7ConversionAttempted: input.profile7ConversionAttempted === true,
        hdr10FallbackAttempted: input.hdr10FallbackAttempted === true,
...
function classifyPlaybackHealth(observation) {
    ...
    var videoHealthy = value.videoStable && value.videoFramePresent && value.controlsResponsive;
...
function selectPlaybackRecovery(observation) {
    ...
    if (!health.videoHealthy && value.dolbyVisionProfile === 7) {
        if (!value.startupTimedOut) return null;
        if (value.canConvertDolbyVisionProfile7 && !value.profile7ConversionAttempted) {
            return 'convertDolbyVisionProfile7To8_1';
        }
        if (value.hdr10BaseLayer && value.canFallbackHdr10 && !value.hdr10FallbackAttempted) {
            return 'fallbackToHdr10BaseLayer';
        }
    }
